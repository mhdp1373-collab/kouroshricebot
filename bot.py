...
def review_keyboard(rid: str) -> InlineKeyboardMarkup:
...
async def receive_barname(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not data["created_at"]:
        ...
        data["driver_id"] = update.effective_user.id
        data["driver_name"] = update.effective_user.full_name
...
async def confirm_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if "documents" not in db_data:
        db_data["documents"] = {}

    db_data["documents"][key] = {
    ...
    if STORAGE_CHANNEL_ID:
        try:
            caption = (
                f"📦 بارنامه: {barname}\n"
                f"📄 نوع: {name}\n"
                f"👤 راننده: {update.effective_user.full_name}\n"
...
async def go_prev_step(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # حذف مستند قبلی اگه ذخیره شده
    barname = context.user_data.get("barname", "")
    prev_key = DOC_STEPS[prev_idx][0]
    db_data = get_barname_data(barname)
    if prev_key in db_data.get("documents", {}):
        del db_data["documents"][prev_key]
        save_barname_data(barname, db_data)
...
async def edit_specific_doc(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    barname = context.user_data.get("barname", "")
    db_data = get_barname_data(barname)
    if doc_key in db_data.get("documents", {}):
        del db_data["documents"][doc_key]
        save_barname_data(barname, db_data)
        ...
        for admin_id in ADMIN_IDS:
            try:
                ...
                sent = await context.bot.send_message(
                    ...
                    text=package_text,
                    parse_mode="Markdown",
                    reply_markup=review_keyboard(rid)
                )
                db_data = get_barname_data(barname)
...
async def review_partial_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=user.id,
        text=(
            f"⚠️ لطفاً *مبلغ/توضیح کسری بار* مربوط به بارنامه {barname} را بنویسید:\n\n"
            "_(مثال: 500,000 تومان بابت ناقص بودن رضایت‌نامه)_"
        ),
        parse_mode="Markdown"
    )
...
def build_pending_list_content():
    """متن و کیبورد لیست بارنامه‌های نیازمند بررسی / تایید نشده"""
    db = load_db()
    rows = []
    for barname, data in db.items():
        review = data.get("review", {})
    ...
    if len(rows) > 25:
        text += f"\n\n_(۲۵ مورد اول از {len(rows)} مورد نمایش داده شد)_"
...
async def admin_pending_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    query = update.callback_query
    await query.answer()
    text, markup = build_pending_list_content()
    await query.edit_message_text(text, parse_mode="Markdown", reply_markup=markup)
...
async def admin_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dispute:
        ...
        await update.message.reply_text(
            "✅ اعتراض شما ثبت و برای ادمین ارسال شد. به‌زودی با شما تماس گرفته می‌شود.",
            reply_markup=driver_reply_keyboard()
        )
        context.user_data.pop("driver_dispute_pending", None)
        return
            ...
            if driver_id:
                try:
                    await context.bot.send_message(
                        chat_id=driver_id,
                        text=(
                            f"⚠️ راننده محترم مستندات ارسالی شما با کسری بار تایید شد "
                            f"که مبلغ {note_text} از هزینه بارنامه کسر و مابقی ظرف 2 روز کاری "
                            "به حساب اعلامی شما واریز خواهد شد.\n\n"
...
async def reply_keyboard_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text == "🚀 آپلود مستندات":
        ...
        if not barname:
            await update.message.reply_text(
                "⚠️ شما در حال حاضر بارنامه فعالی ندارید.\n"
            ...
            done = "\n".join([f"  ✅ {DOC_NAMES.get(k,k)}" for k in docs]) or "  (هیچ‌کدام)"
            await update.message.reply_text(
                f"📦 *وضعیت بارنامه {barname}*\n\n"
                f"📊 {progress_bar(step, len(DOC_STEPS))}\n\n"
                f"✅ آپلودشده:\n{done}",
                parse_mode="Markdown",
                reply_markup=driver_reply_keyboard()
            )
